...
const INSTALL_GUIDANCE =
  "Install nanoclaw-hosthooks@^0.2.0 (API v1 with providerQueryStart) first, then rebuild the host and container image. " +
...
export function requireHosthooks(nanoclawRoot: string): void {
  ...
  throw new Error(
    `nanoclaw-hosthooks API v${EXPECTED_HOSTHOOKS_API_VERSION} is required (${issues.join(
      "; "
    )}). ${INSTALL_GUIDANCE}`
  );
}
